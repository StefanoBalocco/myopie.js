...
// @ts-expect-error — build.mjs is a JS module without declarations
...

{
	...
	test.serial( prefix + ': "all" target should build library and tests', async ( t ) => {
		...
		// Minified artifact smoke test — runs after build completes
		...
		const MyopieMin: any = ( await import( url ) ).default;

		document.body.innerHTML = '<div id="container"></div>';
		const template = ( data: any ): string => `<div>${data.content}</div>`;
		const myopie: any = new MyopieMin( '#container', template, { content: 'hello' } );
	...
	test.serial( prefix + ': "all library library" should deduplicate targets and build in order', async ( t ) => {
		t.timeout( 120000 );
		const { stdout }: { stdout: string } = await execFileAsync( 'node', [ 'build.mjs', 'all', 'library', 'library' ], { timeout: 60000 } );
		t.true( stdout.includes( '[LIBRARY]' ) );
		t.true( stdout.includes( '[TESTS]' ) );
		const libraryCompileCount: number = stdout.split( '[LIBRARY] Compiling TypeScript...' ).length - 1;
		const testsCompileCount: number = stdout.split( '[TESTS] Compiling TypeScript...' ).length - 1;
		...
		const libIndex: number = stdout.indexOf( '[LIBRARY]' );
		const testsIndex: number = stdout.indexOf( '[TESTS]' );
	...
	test.serial( prefix + ': "all unknown" should reject with unknown target diagnostic', async ( t ) => {
		let caughtError: { code: number; stdout: string; stderr: string } | undefined;
		try {
			await execFileAsync( 'node', [ 'build.mjs', 'all', 'unknown' ], { timeout: 60000 } );
			t.fail( 'Expected error to be thrown' );
		} catch( err: unknown ) {
			caughtError = err as { code: number; stdout: string; stderr: string };
		}
		if( caughtError ) {
	...
	test.serial( prefix + ': "tests library" should build library before tests despite CLI order', async ( t ) => {
		t.timeout( 120000 );
		const { stdout }: { stdout: string } = await execFileAsync( 'node', [ 'build.mjs', 'tests', 'library' ], { timeout: 60000 } );
		t.true( stdout.includes( '[LIBRARY]' ) );
		t.true( stdout.includes( '[TESTS]' ) );
		const libIndex: number = stdout.indexOf( '[LIBRARY]' );
		const testsIndex: number = stdout.indexOf( '[TESTS]' );
